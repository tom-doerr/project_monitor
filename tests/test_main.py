...
def test_pylint_score_range(_mock_stats):
    ...
    with patch('src.project_watch.main.get_pylint_score', return_value=_mock_stats["pylint"]):
        score = get_pylint_score()
        assert 0.0 <= score <= 10.0

def test_loc_positive(_mock_stats):
    """Verify lines of code count is non-negative."""
    with patch('src.project_watch.main.count_lines_of_code', return_value=_mock_stats["loc"]):
